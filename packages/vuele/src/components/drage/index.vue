<template>
  <div class="celonis-container">
    <div class="button-group">
      <button v-for="button in directionsButtonList" :key="button.value"
        :type="svg.rankdir === button.value ? 'primary' : ''" @click="svg.rankdir = button.value">{{ button.label
        }}</button>
      <button @click="rerender()">重置</button>
      <button @click="createAnimationForVirtualRoutes()">播放</button>
      <button type="primary" @click="setScaleMin()">缩小</button>
      <button type="primary" @click="setScaleMax()">放大</button>
    </div>
  ...
  methods: {
    ...
    setGraphZoom() {
      this.svg.zoom = d3.zoom().on('zoom', () => {
        console.log(d3.zoomTransform(this.svg.chart.node()) , 'this.svg.transform');
        
        this.svg.innerContainer.attr('transform', d3.zoomTransform(this.svg.chart.node()))
      })
...
.celonis-container .range-list {
  margin: 30 0;
}

.celonis-container svg {
