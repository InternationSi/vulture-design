...
const linkedFGMethods = Object.assign(
  ...['refresh', 'getGraphBbox', 'd3Force', 'd3ReheatSimulation', 'emitParticle'].map((p) => ({
...
const linkedRenderObjsProps = Object.assign(
  ...['width', 'height', 'backgroundColor', 'showNavInfo', 'enablePointerInteraction'].map((p) => ({
    [p]: bindRenderObjs.linkProp(p),
  }))
)
const linkedRenderObjsMethods = Object.assign(
  ...['cameraPosition', 'postProcessingComposer'].map((p) => ({ [p]: bindRenderObjs.linkMethod(p) })),
  {
...
export default Kapsule({
  ...
  methods: {
    zoomToFit: function (state, transitionDuration, padding, ...bboxArgs) {
      state.renderObjs.fitToBbox(state.forceGraph.getGraphBbox(...bboxArgs), transitionDuration, padding)
      return this
    },
      ...
      .onHover((obj) => {
        ...
        if (hoverObj !== state.hoverObj) {
          ...
          renderer.domElement.classList[
            (hoverObj && state[`on${objType === 'node' ? 'Node' : 'Link'}Click`]) ||
            (!hoverObj && state.onBackgroundClick)
              ? 'add'
              : 'remove'
          ]('clickable')
