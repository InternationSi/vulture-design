    ...
    dagMode: {
      onChange(dagMode, state) {
        ...
        !dagMode &&
          ...
          (state.graphData.nodes || []).forEach((n) => (n.fx = n.fy = n.fz = undefined)) // unfix nodes when disabling dag mode
      },
    },
    dagLevelDistance: {},
  ...
  methods: {
    ...
    getGraphBbox: function (state, nodeFilter = () => true) {
      ...
      const bboxes = (function getBboxes(obj) {
        ...
        return bboxes.concat(
          ...(obj.children || [])
            .filter(
              (obj) =>
                !obj.hasOwnProperty('__graphObjType') || (obj.__graphObjType === 'node' && nodeFilter(obj.__data)) // exclude filtered out nodes
            )
            .map(getBboxes)
        )
      })(state.graphScene)
      ...
      return Object.assign(
        ...['x', 'y', 'z'].map((c) => ({
          [c]: [d3Min(bboxes, (bb) => bb.min[c]), d3Max(bboxes, (bb) => bb.max[c])],
        }))
      )
    },
  },
      ...
      if (isD3Sim) {
        ...
        if (state.dagMode) {
          const getFFn = (fix, invert) => (node) =>
            !fix ? undefined : (nodeDepths[node[state.nodeId]] - maxDepth / 2) * dagLevelDistance * (invert ? -1 : 1)
