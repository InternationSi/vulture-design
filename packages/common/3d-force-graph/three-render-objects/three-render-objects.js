...
export default Kapsule({
  ...
  methods: {
    ...
    fitToBbox: function (state, bbox, transitionDuration = 0, padding = 10) {
      ...
      if (bbox) {
        ...
        const maxBoxSide =
          Math.max(
            ...Object.entries(bbox).map(([coordType, coords]) =>
              Math.max(...coords.map((c) => Math.abs(center[coordType] - c)))
            )
          ) * 2
        ...
        if (distance > 0) {
          const newCameraPosition = center.clone().sub(camera.position).normalize().multiplyScalar(-distance)
  ...
  init(
    domNode,
    state,
    { controlType = 'trackball', rendererConfig = {}, extraRenderers = [], waitForLoadComplete = true } = {}
  ) {
    // Wipe DOM
    domNode.innerHTML = ''
        ...
        (ev) => {
          ...
          !state.isPointerDragging &&
            ...
            (ev.pointerType !== 'touch' ||
              ...
              [ev.movementX, ev.movementY].some((m) => Math.abs(m) > 1)) && // relax drag trigger sensitivity on touch events
    ...
    ;[state.renderer, state.postProcessingComposer, ...state.extraRenderers].forEach((r) =>
  ...
  update(state, changedProps) {
    ...
    if (
      state.width &&
      state.height &&
      (changedProps.hasOwnProperty('width') || changedProps.hasOwnProperty('height'))
    ) {
    ...
    if (changedProps.hasOwnProperty('skyRadius') && state.skyRadius) {
      state.controls.hasOwnProperty('maxDistance') &&
        changedProps.skyRadius &&
